...
def plot_training(
        epochs_train, epochs_test, loss_graph_train, loss_graph_train_hybrid,
        loss_graph, auc_graph_train, train_accuracy, auc_graph_train_puppi, train_accuracy_puppi,
        train_metricPUPPI, test_metricPUPPI, train_metricSSL, test_metricSSL,
        loss_graph_test, loss_graph_test_hybrid, auc_graph_test, test_accuracy, auc_graph_test_puppi, test_accuracy_puppi,
    ...
    plt.figure()
    #plt.plot(epochs_train, loss_graph, label='train', linestyle='solid', linewidth=1, color='r')
    plt.plot(epochs_test, loss_graph_train_hybrid, label = 'train_avg', linestyle = 'solid', linewidth = 1, color = 'g')
    plt.plot(epochs_test, loss_graph_test_hybrid, label='Validation', linestyle='solid', linewidth=1, color='b')
    plt.xlabel('Epochs')
    plt.ylabel('loss_da')
    plt.legend(loc=4)
    plt.savefig(dir_name + "/loss_graph_da" + postfix)
    plt.close()

    plt.figure()
    #plt.plot(epochs_train, loss_graph, label='train', linestyle='solid', linewidth=1, color='r')
    plt.plot(epochs_test, train_metricPUPPI, label = 'train_PUPPI', linestyle = 'solid', linewidth = 1, color = 'g')
    plt.plot(epochs_test, train_metricSSL, label = 'train_SSL', linestyle = 'solid', linewidth = 1, color = 'r')
    plt.xlabel('Epochs')
    plt.ylabel('sigma/(1-|mu|)')
    plt.legend(loc=4)
    plt.savefig(dir_name + "/physic_metrics_train" + postfix)
    plt.close()

    plt.figure()
    plt.plot(epochs_test, test_metricPUPPI, label='Valid_PUPPI', linestyle='solid', linewidth=1, color='b')
    plt.plot(epochs_test, test_metricSSL, label='Valid_SSL', linestyle='solid', linewidth=1, color='y')
    plt.xlabel('Epochs')
    plt.ylabel('sigma/(1-|mu|)')
    plt.legend(loc=4)
    plt.savefig(dir_name + "/physic_metrics_valid" + postfix)
    plt.close()
